...
from logic.todo_logic import add_activity, list_activities, delete_all_activities, delete_activity_by_index, update_activity
...
class TodoApp(ctk.CTk):
    def __init__(self):
        ...
        self.geometry("1000x700")  # Agrandir la fenêtre pour inclure la sidebar
    ...
    def create_widgets(self):
        # Conteneur principal avec une barre latérale et un contenu principal
        main_frame = ctk.CTkFrame(self, fg_color="transparent")
        main_frame.pack(fill="both", expand=True)

        # Barre latérale (Sidebar)
        self.sidebar = ctk.CTkFrame(main_frame, width=200, fg_color=self.colors["card"])
        self.sidebar.pack(side="left", fill="y")

        # Bouton dans la sidebar pour afficher/masquer le contenu
        self.toggle_button = ctk.CTkButton(
            self.sidebar,
            text="Afficher/Masquer 📅",
            command=self.toggle_main_content,
            font=("Arial", 14),
            fg_color=self.colors["accent"],
            hover_color="#2980B9"
        )
        self.toggle_button.pack(pady=20, padx=10, fill="x")

        # Contenu principal (initialisé comme visible)
        self.main_content = ctk.CTkFrame(main_frame, fg_color="transparent")
        self.main_content.pack(side="right", fill="both", expand=True)

        # Widgets du contenu principal
        self.create_main_content()

    def toggle_main_content(self):
        """Affiche ou masque le contenu principal."""
        if self.main_content.winfo_ismapped():
            self.main_content.pack_forget()  # Masque le contenu principal
        else:
            self.main_content.pack(side="right", fill="both", expand=True)  # Affiche le contenu principal


    def edit_selected_activity(self):
        """Pré-remplit le formulaire avec les détails de l'activité sélectionnée pour modification."""
    ...
    def create_main_content(self):
        """Crée les widgets du contenu principal."""
        # Conteneur déroulant pour le contenu principal
        scrollable_frame = ctk.CTkScrollableFrame(self.main_content, fg_color="transparent")
    ...
    def reset_form(self):
        ...
        self.activity_entry.delete(0, "end")    # Efface la description
        self.selected_activity_index = None    # Réinitialise l'index sélectionné
        self.add_button.configure(text="Ajouter Activité ➕")  # Remet le texte du bouton à "Ajouter"
